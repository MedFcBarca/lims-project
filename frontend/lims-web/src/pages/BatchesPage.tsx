...
import { Boxes, Building2, Calendar, Plus, Receipt } from 'lucide-react'
...
export default function BatchesPage() {
  ...
  const generateInvoice = async (id: number) => {
    try {
      await api.post(`/invoices?batchId=${id}`)
      alert('Invoice generated successfully')
    } catch (error: any) {
      alert(error.response?.data || 'Invoice generation failed')
    }
  }
    ...
    <div>
      <div className="mb-8">
        <p className="text-slate-400">
          Group multiple samples under the same laboratory reception
        </p>
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Create a lot before attaching samples to it
              </p>
            </div>
        ...
        {batches.map(batch => (
          ...
          >
            ...
            <div className="mt-5 space-y-3 text-sm text-slate-500">
              ...
              <p>{batch.samplesCount} samples</p>
            ...
            <button
              onClick={e => {
                e.stopPropagation()
                generateInvoice(batch.id)
              }}
              disabled={batch.samplesCount === 0}
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-100 px-5 py-3 font-black text-amber-700 hover:bg-amber-200 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <Receipt size={18} />
              Generate invoice
            </button>
